...
class LogisticRegressor:
    ...
    def predict_proba(self, X):
        ...
        z = self.bias + ((self.weights).T).dot(X.T)
    ...
    def predict(self, X, threshold=0.5):
        ...
        for proba in probabilities:
            if proba >= threshold:
                probabilidades_final.append(1)
            else:
                probabilidades_final.append(0)
        # classification_result = [1 if proba >= threshold else 0 for proba in probabilities]
        classification_result = np.array(probabilidades_final)
    ...
    def elasticnet_regularization(self, dw, m, C, l1_ratio):
        ...
        elasticnet_gradient = l1_ratio * (self.lasso_regularization(dw,m,C) - dw) + (1 - l1_ratio) * (self.ridge_regularization(dw,m,C) - dw) # Le quitamos dw pq ya nos lo devuelven
